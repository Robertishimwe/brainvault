...
function splitText(text, chunkSize) {
  ...
  var start = 0;
  while (start < textLength) {
    var chunk = text.slice(start, start + chunkSize);
    textChunks.push(chunk);
    start += chunkSize - overlap;
  }
  return textChunks;
}
function createEmbeddings(_x) {
  return _createEmbeddings.apply(this, arguments);
}
function _createEmbeddings() {
  _createEmbeddings = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(textChunks) {
    var model,
      embeddings,
      preparedChunks,
      response,
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          model = _args.length > 1 && _args[1] !== undefined ? _args[1] : "text-embedding-ada-002";
          embeddings = [];
          _context.prev = 2;
          preparedChunks = textChunks.map(function (chunk) {
        ...
        case 12:
          ...
          console.log("Error creating embeddings: ".concat(_context.t0));
